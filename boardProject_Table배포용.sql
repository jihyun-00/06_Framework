...
VALUES(SEQ_MEMBER_NO.NEXTVAL, 
			 'admin01@kh.or.kr',
			 '$2a$10$En5p3BPMiilVoZkiZn/4YOtPUOBy6E83m27poNR0.24npvGlDAAb6',
			 '관리일',
			 '01012341234',
			 NULL,
			 NULL,
...
UPDATE "MEMBER" SET
MEMBER_PW='$2a$10$En5p3BPMiilVoZkiZn/4YOtPUOBy6E83m27poNR0.24npvGlDAAb6'
WHERE MEMBER_NO=1;
...
VALUES(1, 2000); -- 1번 회원이 2000번 게시글에 좋아요를 누름
...
-- 작성일 : 몇초 / 몇분 / 몇시간 전 조회, 하루가 넘어가면 YYYY-MM-DD 형식 조회

-- 게시글번호 / 제목[댓글개수] / 작성자닉네임 / 작성일 / 조회수 / 좋아요갯수
...
-- LEVEL : Oracle에서 제공하는 계층형 데이터 조회할 때 "레벨"에 있는지 알려주는 가상의 컬럼
SELECT LEVEL, C.* FROM
...
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT, -- NULL이 아닌 경우 (로그인한 사람 오픈멤버일 때는 참가자 타겟번호)
		R.OPEN_MEMBER -- NULL인 경우 (내가 오픈멤버가 아닐 때는 OPEN_MEMBER가 타겟번호)
		) TARGET_NO	
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		WHERE R2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		AND R2.OPEN_MEMBER = 1),
