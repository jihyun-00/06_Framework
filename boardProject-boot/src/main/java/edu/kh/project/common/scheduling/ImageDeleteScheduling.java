 ...
 *
 *
 *  @Scheduled(cron = "30 * * * * *") // 30초마다 수행
 *  @Scheduled(cron = "0,30 * * * * *") // 0초, 30초마다 수행
 *  @Scheduled(cron = "0 0 12 * * *") // 12시 정오마다 수행
 *  @Scheduled(cron = "0 0 0 * * *") // 0초 0분 0시 모든일 -> 매일 자정마다 수행
 *  @Scheduled(cron = "0 0 * * * *") // 매시간(정시)마다 수행
 *  
 *  
 *
 *
