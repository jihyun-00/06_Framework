...
public class ChattingWebsocketHandler extends TextWebSocketHandler{
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		// {"senderNo": loginMemberNo, // "1"
		// "targetNo": selectTargetNo, // "2"
		// "chattingRoomNo": selectChattingNo, // "8"
		// "messageContent": inputChatting.value} // "안녕!"
		
		// Jackson(JSON과 관련된 모듈)에서 제공하는 객체
		ObjectMapper objectMapper = new ObjectMapper();
		
		Message msg = objectMapper.readValue(message.getPayload(), Message.class); // 읽어들여서 읽어들인 값을 class에 세팅
		...
		if(result > 0) {
			...
			for(WebSocketSession s : sessions) {
				...
				// 로그인 상태인 회원 중 targetNo 또는 sendNo 일치하는 회원에게 메시지 전달
				if(loginMemberNo == msg.getTargetNo() || loginMemberNo == msg.getSenderNo()) {
					...
					s.sendMessage(new TextMessage(jsonData)); // -> 웹소켓컨피그.java로 이동해서 의존성 주입
				}
