...
public class AdminController {
	 ...
	 * @return
	 */
	@GetMapping("newMemberCount")
	public ResponseEntity<Object> newMemberCount() {
		try {
			List<Member> member = service.newMemberCount();
	...
	public ResponseEntity<Object> selectWithdrawnMemberList() {
		...
		try {
			...
			log.info("withdrawnMemberList" + withdrawnMemberList);
	...
	public ResponseEntity<Object> selectDeleteBoardList() {
		...
		try {
			...
			log.info("deleteBoardList" + deleteBoardList);
	...
	@PutMapping("restoreMember")				// 비동기, JSON으로 넘어오기 때문에 리퀘스트바디
	public ResponseEntity<String> restoreMember(@RequestBody Member member) {
		...
		try {
			...
			} else { // 회원번호에 해당하는 결과가 없는 경우
				// BAD_REQUEST : 400 -> 요청 구문이 잘못되었거나 유효하지 않음.
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("유효하지 않은 회원번호 : " + member.getMemberNo());
			}
	...
	@PutMapping("restoreBoard")				// 비동기, JSON으로 넘어오기 때문에 리퀘스트바디
	public ResponseEntity<String> restoreBoard(@RequestBody Board board) {
		...
		try {
			...
			if(result > 0) {
				...
				return ResponseEntity.status(HttpStatus.OK)
						.body(board.getBoardNo() + "번 회원 복구 완료");
				
			} else { // 회원번호에 해당하는 결과가 없는 경우
				// BAD_REQUEST : 400 -> 요청 구문이 잘못되었거나 유효하지 않음.
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("유효하지 않은 회원번호 : " + board.getBoardNo());
			}
