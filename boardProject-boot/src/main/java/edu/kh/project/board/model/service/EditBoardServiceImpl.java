...
public class EditBoardServiceImpl implements EditBoardService{
	...
	@Value("${my.board.web-path}") // /images/board/
	...
	public int boardInsert(Board inputBoard, List<MultipartFile> images)
		...
		// 다중 INSERT 성공 확인
		// (uploadList에 저장된 값이 모두 정상 삽입되었는가)
	...
	public int boardUpdate(Board inputBoard, List<MultipartFile> images, String deleteOrderList) throws Exception {
		...
		// 2. 기존에 있었으나 삭제된 이미지
		// == 기존 0 -> 삭제된 이미지(deleteOrderList)가 있는 경우
		...
		for(int i=0; i<images.size(); i++) {
			...
			if(!images.get(i).isEmpty()) {
				...
				BoardImg img = BoardImg.builder()
							   .imgOriginalName(originalName)
							   .imgRename(rename)
							   .imgPath(webPath) // 위에 선언해놓은 webPath
		...
		for(BoardImg img : uploadList) {
			img.getUploadFile().transferTo(new File(folderPath + img.getImgRename())); // folderPath : 위에서 선언
		}
