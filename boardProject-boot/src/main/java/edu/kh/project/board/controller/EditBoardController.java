...
public class EditBoardController {
	...
	public String boardInsert(@PathVariable("boardCode") int boardCode,
		 ...
		 * - 5개 모두 업로드 0 -> List안에 0~4번 인덱스에 파일 저장됨
		 * - 5개 모두 업로드 X -> List안에 0~4번 인덱스에 파일 저장 X -> null 상태
	...
	public String boardUpdate(@PathVariable("boardCode") int boardCode,
		...
		} else if(board.getMemberNo() != loginMember.getMemberNo()) { // 현재 로그인한 사람 != 작성자
	...
	public String boardDelete(@PathVariable("boardCode") int boardCode,
		...
		if(result > 0) {
			...
			path = String.format("/board/%d/%d?cp=%d", boardCode, boardNo, cp); // 게시글 상세 조회
			// /board/1/2000?cp=7
		}
