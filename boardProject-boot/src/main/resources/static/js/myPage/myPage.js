...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async e => {
        ...
        /*
        submit 이벤트 핸들러 함수 내에서 fetch를 이용한 비동기 함수를 실행 중일 때
        e.preventDefault()가 비동기 함수 이후에 호출되면 이미 form이 submit 되어버린 다음일 수 있음
        -> 비동기 작업 중에 fotm의 기본 제출 동작 일어나는 가능성 생김!
        */
    });
}
...
if(profileForm != null) {
    ...
    deleteImage.addEventListener("click", () => {
        ...
        if(profileImg.src !== defaultImageUrl) {
            ...
            previousFile = null; // 이전 파일 기록하는 변수에 null 초기화

        } else {

            // 기본 이미지 상태에서 삭제 버튼 클릭 시 상태 변경하지 않음
