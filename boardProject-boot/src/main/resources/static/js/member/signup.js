...
// 매개변수로 전달받은 숫자가 10 미만인 경우(한자리) 앞에 0 붙여서 반환
...
checkAuthKeyBtn.addEventListener("click", () => {
  if( min == 0 && sec == 0 ) { // 타이머가 00:00인 경우
    alert("인증번호 입력 제한시간을 초과하였습니다. 다시 발급받아주세요.");
    return;
  }

  if(authKey.value.length < 6 || authKey.value.length >= 7) { // 인증번호가 제대로 입력 안된 경우 (길이가 6미만 또는 7이상)
...
signUpForm.addEventListener("submit", (e) => {
  ...
  // checkobj의 저장된 값 중
  // 하나라도 false가 있으면 제출 막음
  // for ~ in (객체 전용 향상된 for문)
  for(let key in checkobj) { // checkobj 요소의 key값을 순서대로 꺼내옴
    if(!checkobj[key]) { // 현재 접근 중인 checkobj[key]의 value 값이 false인 경우
