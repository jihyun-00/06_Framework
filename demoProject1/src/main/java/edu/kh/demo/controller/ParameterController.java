...
public class ParameterController {
	 ...
	 * -> ""(빈문자열)일 때는 에러 발생 X
	 * (파라미터가 존재하지 않는 것이 아니라 name 속성값 = "" 로 넘어오기 때문에)
	 * 
	...
	@PostMapping("test2") // param/test2 POST 방식 요청 매핑하는 메서드다!
	public String paramTest2(@RequestParam("title") String title,
							@RequestParam("writer") String writer,
							@RequestParam("price") int price, // 형변환이 필요한 경우에는 파라미터 value 값이 필수 작성되어 있어야 한다!
							@RequestParam(value="pyublisher", required=false, defaultValue="kh풀판사") String publisher) {
	...
	public String paramTest3(@RequestParam("color") String[] colorArr,
		...
		// -> 같은 name 속성을 가진 파라미터는 String[], List로 저장 X
		// 	  첫번째로 제출된 value값이 저장됨
		log.debug("paramMap : " + paramMap);
		// paramMap : {color=Red, fruit=Apple, productName=아름다운조명, expirationDate=2025-04-04}
	...
	// 자동으로 setter를 호출해서 필드에 값 저장
	
	// ** 주의사항 **
	// - DTO에 기본 생성자 필수로 존재해야 한다!
	...
	public String paramTest4(/*@ModelAttribute*/ MemberDTO inputMember) {
		...
		return "redirect:main"; // 상대경로 작성법 (현재 위치가 중요! -> 현재 경로의 가장 마지막 부분을 redirect:이 주소로 변경하기 때문에)
		// == "redirect:/param/main"
	}
